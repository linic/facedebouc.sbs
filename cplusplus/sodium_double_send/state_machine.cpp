...
StateMachine::StateMachine(int socket, struct sockaddr_in& remote) 
    : current_state(State::Initialization), sock(socket), remote_addr(remote), 
      received_packets(10), sending_key(true), listening(true) {}
...
void StateMachine::processAndSendInput(const std::string& input, const unsigned char* tx_key) {
    ...
    crypto_secretbox_easy(reinterpret_cast<unsigned char*>(encrypted),
                          ...
                          input.length(), nonce, tx_key);
    print_as_hex(encrypted, input.length());
    std::cout << "//encrypted\n";
    
    std::vector<char> message(crypto_secretbox_NONCEBYTES + input.length() + crypto_secretbox_MACBYTES);
    std::memcpy(message.data(), nonce, crypto_secretbox_NONCEBYTES);
    std::memcpy(message.data() + crypto_secretbox_NONCEBYTES, encrypted, input.length() + crypto_secretbox_MACBYTES);
    ...
    while (listening) {
        ...
        if (packet.size() > crypto_secretbox_NONCEBYTES + crypto_secretbox_MACBYTES) {
            ...
            if (crypto_secretbox_open_easy(reinterpret_cast<unsigned char*>(decrypted),
                ...
                std::cout.write(decrypted, decrypted_len);
                std::cout << std::endl;
                std::cout.flush();
            } else {
